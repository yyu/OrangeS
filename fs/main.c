...
/* Orange'S FS illustration
	         ...
	         NrSectors/4096+1 sectors = 0x9D41/4096+1 = 10
	         ------------------------
	                   |
	                   |      256 sectors(4096/16)
	                   |      --------------------
	...
	+---+---+---+---+-...-+---+---+-...-+---+---+---+-...
	 ...
	 |    |   |        |             |             |
	 |    |   |        |             |            data (sect269~...)
	 |    |   |        |            inode_array (sect13~268)
	 |    |   |       sector map (sect3~12)
	 |    |   |
	 |    |  inode map (sect2) (4096 inodes mapped)
	 |    |
	 |   superblock (sect1)
	 |
	boot sector (sect0)
	...
	inode_array:
		...
		+---+-...-+---+
		  |
		  |
		 16 inodes per sector (16 = 512 / INODE_SIZE)
		                                                                                    
		INODE_SIZE = 32 != sizeof(inode)


	`/' (root):
		ROOT_INODE      :  1   ← zero-based, 0 is reserved
		bit_nr in imap  :  1   ← zero-based, bit 0 is always set to 1
		inode_array[?]  :  0   ← zero-based, put root inode into the
		                          first slot in case of wasting
	therefore:
		                                / bit M in imap
		inode M        corresponds with |
		                                \ inode_array[M-1]

		                                / inode M
		bit M in imap  corresponds with |
		                                \ inode_array[M-1]

		                                / inode M+1
		inode_array[M] corresponds with |
		                                \ bit M+1 in imap

	similarly:
	for the first sect of ROOT (data area):
		sector nr      : super_block.n_1st_sect
		bit_nr in smap : 1
	easy to know:
		sector M      → bit (M - super_block.n_1st_sect + 1) in smap
		bit M in smap → sector (M - 1 + super_block.n_1st_sect)

	things are the same in MINIX:
		Note that the routine alloc_bit() returns 1 for the lowest
		possible zone, which corresponds to sb->s_firstdatazone.
		To convert a value between the bit number, 'b', used by
		alloc_bit() and the zone number, 'z', stored in the inode,
		use the formula:
		    z = b + sb->s_firstdatazone - 1
		Alloc_bit() never returns 0, since 0 is NO_BIT (failure).

					---- .../src/fs/cache.c::ln198
...
PRIVATE void mkfs()
{
	...
	assert(sb.magic == 0x111);
	assert(sb.nr_inodes == 4096);
	assert(sb.nr_inode_sects == (sb.nr_inodes * INODE_SIZE) / SECTOR_SIZE);
	assert(sb.nr_imap_sects == 1);
	assert(sb.nr_smap_sects == (sb.nr_sects / (SECTOR_SIZE * 8) + 1));
...
PRIVATE int fs_fork()
{
	...
	/* dump_inode_table(); */
	/* dump_fd_graph("fs_fork: %s(%d) just born.", child->name, fs_msg.PID); */
...
PUBLIC void dump_inode_table()
{
	...
	SYSLOG("---------- inode_table dump (in HEX):\n");
	for (p = &inode_table[0]; p < &inode_table[NR_INODE]; p++,i++) {
		SYSLOG("[%3d]%x::mode(%x),size(%x),start(%x),nr(%x),dev(%x),cnt(%x),num(%x)\n",
		       i,
		       (int)p,
		       p->i_mode,
		       p->i_size,
