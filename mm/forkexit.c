...
PUBLIC void do_fork_test()
{
	...
	/* save ldt_sel (it was set by protect.c) */
	u16 child_ldt_sel = proc_table[child_pid].ldt_sel;
	proc_table[child_pid] = proc_table[pid];
	proc_table[child_pid].ldt_sel = child_ldt_sel; /* recover ldt_sel */

	sprintf(proc_table[child_pid].name, "%s_%d",
		proc_table[pid].name, child_pid);

	phys_copy((void*)0x800000, (void*)0, 0x60000);
...
/*
              ...
              H +----------------------------+                                    |                            |                      |    |                    |  |		   
                ...
                |                            |                  /                 ~     STACK FRAME            ~  |                   |    |                    |
                |                           -+-----------------`                  |                            |  |                    \ L +--------------------+
        .-----> |     ldt desc for proc i    |    initialized                   L +----------------------------+  |                             process image
        |       +----------------------------+        in                                   proc_table[i]          |                         allocated in do_fork()
        |       |                            |    protect.c::                                                     |           
        |       ~             ~              ~    init_descriptor(&gdt[INDEX_LDT_FIRST + i],                      |           
        |       |                            |                    makelinear(SELECTOR_KERNEL_DS,                  |
        |     L +----------------------------+                               proc_table[i].ldts),                 |
        |                    GDT                                  LDT_SIZE * sizeof(DESCRIPTOR) - 1,              |
        |                                                         DA_LDT);                                        |
        |                                                                                                         |
        |                                                                                                         |
        `---------------------------------------------------------------------------------------------------------`
...
*/
	...
	*p = proc_table[pid];
	...
	printl("ppd->limit_high_attr2: 0x%x\n", ppd->limit_high_attr2);
	...
	init_desc(&p->ldts[INDEX_LDT_C],
		  child_base,
		  (PROC_IMAGE_SIZE_DEFAULT - 1) >> LIMIT_4K_SHIFT, /* child_limit */
...
PUBLIC void do_exit(int status)
{
	...
	if (proc_table[parent_pid].p_flags & WAITING) { /* parent is waiting */
		printl("((--do_exit():: %s (%d) is WAITING, %s (%d) will be cleaned up.--))\n",
		...
		printl("((--do_exit():1: proc_table[parent_pid].p_flags: 0x%x--))\n",
		...
		printl("((--do_exit():: %s (%d) is not WAITING, %s (%d) will be HANGING--))\n",
		       proc_table[parent_pid].name, parent_pid,
		       p->name, pid);
	...
	for (i = 0; i < NR_TASKS + NR_PROCS; i++) {
		if (proc_table[i].p_parent == pid) { /* is a child */
			...
			printl("%s (%d) exit(), so %s (%d) is INIT's child now\n",
			       p->name, pid, proc_table[i].name, i);
			/* dump_fd_graph("%s (%d) exit(), so %s (%d) is INIT's child now", */
			/*        p->name, pid, proc_table[i].name, i); */
			printl("((--do_exit():2: proc_table[INIT].p_flags: 0x%x--))\n",
...
PRIVATE void cleanup(struct proc * proc)
{
	...
	printl("((--cleanup():: %s (%d) has been cleaned up.--))\n", proc->name, proc2pid(proc));
...
PUBLIC void do_wait()
{
	...
	if (children) {
		...
		printl("((--do_wait():: %s (%d) is WAITING for child "
		...
		/* no child at all */
		printl("((--do_wait():: %s (%d) has no child at all.--))\n",
		       proc_table[pid].name, pid);
